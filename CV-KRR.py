...
def _get_KRR_prediction(data_, theta_, kernel, degree):
...
def _KFold_CV(data_, theta_, kernel, degree, n_kfolds):
    ...
    for idx_val_tr_, idx_val_ts_ in KFold(n_splits     = n_kfolds,
        ...
        #try:
        # Training and Testing KRR
        Y_val_ts_hat_prime_ = _get_KRR_prediction(data_val_, theta_, kernel, degree)[0]
...
# KRR K-Fold Cross-Validation of the model Parameters
...
def _KRR_traing_and_testing(data_, theta_, kernel, degree):
    ...
    #try:
    # Training and testing KRR
    Y_ts_hat_prime_, time_, _KRR = _get_KRR_prediction(dataset_, theta_, kernel, degree)
    # Undo Normalization of the prediction
    ...
    #     print(theta_)
    #     e_[j] = 1e10
    return e_ts_, Y_ts_hat_, time_, [_KRR, scaler_]
...
# Save Errors
name = r'{}/logs/kernel_learning/KRRs/CV-KRR_v31-1_{}.csv'.format(root, i_label)
with open(name, 'a', newline = '\n') as f:
    writer = csv.writer(f)
    writer.writerow([i_ker, i_cov, i_sec, i_nor] + time_ + e_val_machine_.tolist() + e_ts_machine_.tolist())
# Save Models
name = r'{}/model/kernel_learning/KRRs/CV-KRR_v31-1_{}{}{}{}-{}.pkl'.format(root, i_ker, i_cov, i_sec, i_nor, i_label)
with open(name, 'wb') as handle:
    pickle.dump(_model, handle, protocol = pickle.HIGHEST_PROTOCOL)
# Save Results
name = r'{}/data/kernel_learning/KRRs/CV-KRR_v31-1_{}{}{}{}-{}.pkl'.format(root, i_ker, i_cov, i_sec, i_nor, i_label)
with open(name, 'wb') as handle:
