...
def _KRR_fit(X_, y_, kernel, degree, gamma, beta, alpha, weights = None):
    # Numpy to Torch
    X_ = torch.tensor(X_, dtype = torch.float)
    y_ = torch.tensor(y_, dtype = torch.float)
    # Evaluate kernel function for training
    K_ = _kernel(X_, X_, kernel, degree, gamma = gamma, beta = beta)
    # KRR Least-Square Solution
    a_star_ = _robust_pinverse(K_ + alpha*torch.eye(K_.shape[0])) @ y_
    return [a_star_, X_, [kernel, degree, gamma, beta]]

# Calculing prediction for new sample
def _KRR_predict(Y_, model_):
    # Numpy to Torch
    Y_ = torch.tensor(Y_, dtype = torch.float)
    # Model parameters and hyperparameters
    a_star_, X_, theta_         = model_
    kernel, degree, gamma, beta = theta_
    # Evaluate kernel function for Testing
    K_star_     = _kernel(X_, Y_, kernel, degree, gamma = gamma, beta = beta)
    # Make a prediction
    y_star_hat_ = K_star_.t() @ a_star_
    # Torch to Numpy
    return np.squeeze(y_star_hat_.detach().numpy())

# Kernel Ridge Regression Chain
def _get_KRR_prediction_chain(data_, theta_, kernel, degree):
    ...
    for i_task in range(N_tasks):
        ...
        if theta_.shape[0] == N_tasks:
            # Define Model
            _KRR = _KRR_fit(x_tr_, y_tr_, kernel = kernel,
                                          degree = degree,
                                          alpha  = theta_[i_task, 0],
                                          gamma  = theta_[i_task, 1],
                                          beta   = theta_[i_task, 2])
        else:
            # Define Model
            _KRR = _KRR_fit(x_tr_, y_tr_, kernel = kernel,
                                          degree = degree,
        ...
        # Testing Model
        t_init               = time.time()
        y_ts_hat_[:, i_task] = _KRR_predict(x_ts_, _KRR)
    ...
    return y_ts_hat_, [t_tr, t_ts], _model
...
def _KFold_CV(data_, theta_, kernel, degree, n_kfolds):
    ...
    for idx_val_tr_, idx_val_ts_ in KFold(n_splits     = n_kfolds,
        ...
        #try:
        # Training and Testig KRR
        Y_val_ts_hat_prime_ = _get_KRR_prediction_chain(data_val_, theta_, kernel, degree)[0]
...
# eKRR K-Fold Cross-Validation of the model Parameters
def _get_KRR_cross_validation(data_, kernel, degree, n_grid, n_kfolds):
    # Define KRR parameters to validate
    alpha_  = np.logspace(-7, 7, n_grid)
    gamma_  = np.array((0., ))
    beta_   = np.array((0., ))
    scale_  = np.array((0., ))
    if kernel == 'linear':
        gamma_ = np.logspace(-2, 2, n_grid)
...
# KRR Model validation without kernels
def _meta_KRR_cross_validation(data_, kernel, degree, n_grid, n_kfolds):
    # KRR Parameters Cross-validation
...
def _KRR_traing_and_testing(data_, theta_, kernel, degree):
    ...
    #try:
    # Training and testing KRR Chain
    Y_ts_hat_prime_, time_, _RKRR = _get_KRR_prediction_chain(dataset_, theta_, kernel, degree)
    ...
    #     print(theta_)
    #     e_[j] = 1e10
    return e_ts_, Y_ts_hat_, time_, [_RKRR, scaler_]
...
# Save Errors
name = r'{}/logs/kernel_learning/KRRs/CV-RKRR_v31-1_{}.csv'.format(root, i_label)
with open(name, 'a', newline = '\n') as f:
    writer = csv.writer(f)
    writer.writerow([i_ker, i_cov, i_sec, i_nor] + time_ + e_val_machine_.tolist() + e_ts_machine_.tolist())
# Save Models
name = r'{}/model/kernel_learning/KRRs/CV-RKRR_v31-1_{}{}{}{}-{}.pkl'.format(root, i_ker, i_cov, i_sec, i_nor, i_label)
with open(name, 'wb') as handle:
    pickle.dump(_model, handle, protocol = pickle.HIGHEST_PROTOCOL)
# Save Results
name = r'{}/data/kernel_learning/KRRs/CV-RKRR_v31-1_{}{}{}{}-{}.pkl'.format(root, i_ker, i_cov, i_sec, i_nor, i_label)
with open(name, 'wb') as handle:
