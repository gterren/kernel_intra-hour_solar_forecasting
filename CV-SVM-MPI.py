...
# Independet Support Vector Machine
def _get_SVM_prediction(data_, theta_, kernel, degree):
...
def _KFold_CV(data_, theta_, kernel, degree, n_kfolds):
    ...
    for idx_val_tr_, idx_val_ts_ in KFold(n_splits     = n_kfolds,
        ...
        for i_task in range(N_tasks):
            ...
            # Get Outliers Index
            outliers_idx_val_tr_ = _get_outliers_index(X_val_tr_, n_samples = 3000)
            ...
            print(i_task, X_val_tr_prime_.shape, Y_val_tr_prime_.shape, X_val_ts_prime_.shape, Y_val_ts_.shape)
            # Save Dataset
        ...
        Y_val_ts_hat_prime_ = _get_SVM_prediction(data_val_, theta_, kernel, degree)[0]
...
def _SVM_traing_and_testing(data_, theta_, kernel, degree):
    ...
    #try:
    # Train and predict SVM
    Y_ts_hat_prime_, time_, _SVM = _get_SVM_prediction(dataset_, theta_, kernel, degree)
    ...
    #     print(theta_)
    #     e_[j] = 1e10
    return e_ts_, Y_ts_hat_, time_, [_SVM, scaler_]
...
if i_job == 0:
    ...
    # Save Errors
    name = r'{}/logs/kernel_learning/SVMs/CV-SVM_v31-1_{}.csv'.format(root, i_label)
    with open(name, 'a', newline = '\n') as f:
        writer = csv.writer(f)
        writer.writerow([i_ker, i_cov, i_sec, i_nor] + time_ + e_val_machine_.tolist() + e_ts_machine_.tolist())
    # Save Models
    name = r'{}/model/kernel_learning/SVMs/CV-SVM_v31-1_{}{}{}{}-{}.pkl'.format(root, i_ker, i_cov, i_sec, i_nor, i_label)
    with open(name, 'wb') as handle:
        pickle.dump(_model, handle, protocol = pickle.HIGHEST_PROTOCOL)
    # Save Results
    name = r'{}/data/kernel_learning/SVMs/CV-SVM_v31-1_{}{}{}{}-{}.pkl'.format(root, i_ker, i_cov, i_sec, i_nor, i_label)
    with open(name, 'wb') as handle:
