...
# Relevance Vector Machine for Regression Chain
def _get_RVM_prediction_chain(data_, theta_, kernel, degree):
    N_tasks = len(data_)
    ...
    for i_task in range(N_tasks):
        ...
        _model.append(_RVM)
    return Y_ts_hat_, S2p_ts_hat_, [t_tr, t_ts], _model
...
def _KFold_CV(data_, theta_, kernel, degree, n_kfolds):
    ...
    for idx_val_tr_, idx_val_ts_ in KFold(n_splits     = n_kfolds,
        ...
        #try:
        # Training and Testing RRVM
        Y_val_ts_hat_prime_ = _get_RVM_prediction_chain(data_val_, theta_, kernel, degree)[0]
...
def _RVM_traing_and_testing(data_, theta_, kernel, degree):
    ...
    #try:
    # Training and testing RRVM
    Y_ts_hat_prime_, S2p_ts_hat_prime_, time_, _RRVM = _get_RVM_prediction_chain(dataset_, theta_, kernel, degree)
    # Undo Normalization of the prediction
    ...
    #     print(theta_)
    #     e_[j] = 1e10
    return e_ts_, Y_ts_hat_, Sp_ts_hat_,time_, [_RRVM, scaler_]
...
# Save Errors
name = r'{}/logs/kernel_learning/RVMs/CV-RRVM_S0_v31-1_{}.csv'.format(root, i_label)
with open(name, 'a', newline = '\n') as f:
    writer = csv.writer(f)
    writer.writerow([i_ker, i_cov, i_sec, i_nor] + time_ + e_val_machine_.tolist() + e_ts_machine_.tolist())
# # Save Models
# name = r'{}/model/kernel_learning/RVMs/CV-RRVM_v31-1_{}{}{}{}-{}.pkl'.format(root, i_ker, i_cov, i_sec, i_nor, i_label)
# with open(name, 'wb') as handle:
#     pickle.dump(_model, handle, protocol = pickle.HIGHEST_PROTOCOL)
# Save Results
name = r'{}/data/kernel_learning/RVMs/CV-RRVM_S0_v31-1_{}{}{}{}-{}.pkl'.format(root, i_ker, i_cov, i_sec, i_nor, i_label)
with open(name, 'wb') as handle:
